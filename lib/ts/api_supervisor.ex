defmodule TS.API.Supervisor do
	@moduledoc """
	Module for implementing API process supervision functionality. Supervisor process supervises
	other processes, which we refer to as child processes. In the event of API process crashing, the
	TS.API.Supervisor is responsible for restarting it based on the exit reasons and strategies mentioned.
	...
	To start the supervisor, the `init/1` callback will be invoked in the given module, with arg
	as its argument. The `init/1` callback must return a supervisor specification which can be
	created with the help of the functions in the Supervisor.Spec module.
   
	### Parameters
	For API, required parameters passed to Supervisor function are `Supervisor.start_link(__MODULE__, 
	...
	### Parameters
	For API, required parameters passed to Supervisor function are `Supervisor.start_child(@name, [])`,
	specifying:
