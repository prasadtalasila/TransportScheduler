defmodule TS.Supervisor do
	@moduledoc """
	Module for implementing supervision functionality. Supervisor process supervises other processes, 
	which we refer to as child processes. In the event of NC process crashing or other child Supervisors
	crashing, TS.Supervisor is responsible for restarting it based on the exit reasons and strategies
	mentioned.
	...
	as its argument. The `init/1` callback must return a supervisor specification which can be created
	with the help of the functions in the Supervisor.Spec module.
   
	### Parameters
	For TS, required parameters passed to Supervisor function are `Supervisor.start_link(__MODULE__, 
	:ok)`, specifying:
