defmodule Station.Fsm do
	...
	defp query_status(station_vars, itinerary, dependency) do
		...
		cond do
			...
			(active && itinerary_fn.is_terminal(itinerary)) ->
				:collect
			(!active ||
			!itinerary_fn.is_valid_destination(station_vars.station_number, itinerary))
			->	:invalid
			true ->
				:valid
		end
	...
	defstate ready do
		...
		dependency]do
			...
			new_station_vars = %Util.StationStruct{loc_vars: new_vars.loc_vars,
			 schedule: schedule,
	...
	defstate query_init do
		defevent initialise, data: vars = [itinerary, station_vars, dependency]
		 do
