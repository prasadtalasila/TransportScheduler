defmodule TS.Station.Supervisor do
	@moduledoc """
	Module for implementing station process supervision functionality.
	...
	exit reasons and strategies mentioned.
	...
	given module, with arg as its argument. The `init/1` callback must
	return a supervisor specification which can be created with the	help of
	the functions in the Supervisor.Spec module.

	### Parameters
	For Station, required parameters passed to Supervisor function are
	`Supervisor.start_link(__MODULE__, :ok, name: @name)`, specifying:
	...
	- :shutdown or {:shutdown, term} - in such cases, the exit won't be logged,
	...
	list to the existing function arguments in the child specification.
	...
	about the error and child specification.
	"""
